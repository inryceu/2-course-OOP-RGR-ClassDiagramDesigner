...
import { parseVisibility, parseCSharpParameters, addConstructorToClass, filterControlFlowKeywords } from '../utils/parserHelpers.js';
...
export class CSharpParser extends BaseParser {
        ...
        while ((match = interfacePattern.exec(code)) !== null) {
            ...
            if (inheritanceStr) {
                ...
                interfaces.forEach(parentInterface => {
                    diagram.addRelationship(new Relationship(interfaceName, parentInterface, RelationType.INHERITANCE));
                });
            }
        }
    }
    extractBalancedBraces(code, startIndex) {
        let braceCount = 1;
        let i = startIndex;
        let inString = false;
        let stringChar = '';
    ...
    parseInheritance(className, inheritanceStr, diagram) {
        ...
        for (const part of parts) {
            ...
            diagram.addRelationship(new Relationship(className, part, relationType));
        }
    }
}
