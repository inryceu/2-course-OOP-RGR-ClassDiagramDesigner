...
export function groupRelationships(relationships, classPositions) {
...
export function getConnectionsForClass(className, relationships, direction) {
    if (direction === 'parent') {
...
export function countConnectionsForClass(className, relationships) {
    return relationships.filter(rel => rel.from === className || rel.to === className).length;
}
export function countCrossings(items, idx1, idx2, relationships, classPositions) {
    let crossings = 0;
    const class1 = items[idx1].name;
    const class2 = items[idx2].name;
    const connections1 = relationships.filter(rel => rel.from === class1 || rel.to === class1);
    const connections2 = relationships.filter(rel => rel.from === class2 || rel.to === class2);
...
export function optimizeClassOrdering(hierarchy, relationships) {
...
export function calculateOptimizedXPosition(className, defaultX, classWidth, relationships, classPositions, pullStrength = 0.45) {
