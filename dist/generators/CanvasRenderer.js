...
import { resolveOverlaps, calculateConnectionDensity, centerElementsHorizontally, minimizeCrossings, buildClassHierarchy, } from '../utils/layoutUtils.js';
import { drawArrowByType, drawLabel, findBestLabelPosition, drawStartPoint, getRelationshipLabel } from '../utils/renderingUtils.js';
import { drawClassBox, drawClassHeader, drawFields, drawMethods, calculateClassHeight, setupCanvas } from '../utils/canvasDrawingUtils.js';
import { calculateOrthogonalPath, findObstructionBetween } from '../utils/pathUtils.js';
import { groupRelationships, getConnectionsForClass, countConnectionsForClass, countCrossings, optimizeClassOrdering, calculateOptimizedXPosition } from '../utils/relationshipUtils.js';
export class CanvasRenderer {
    ...
    calculatePositions(classes) {
        ...
        const margin = 80;
        const orderedHierarchy = hierarchy.map(level => this.sortLevelByConnections(level));
        ...
        orderedHierarchy.forEach((level, levelIndex) => {
            ...
            level.forEach(classInfo => {
                const height = calculateClassHeight(classInfo, this.config.headerHeight, this.config.lineHeight, this.config.sectionPadding, this.config.padding, this.config.minClassHeight);
            ...
            level.forEach((classInfo, index) => {
                ...
                const optimizedX = levelIndex === 0 ? baseX : calculateOptimizedXPosition(classInfo.name, baseX, this.config.classWidth, this.currentRelationships, this.classPositions);
    ...
    sortLevelByConnections(level) {
        ...
        return minimizeCrossings(sorted, (items, i, j) => countCrossings(items, i, j, this.currentRelationships, this.classPositions));
    }
    clearCanvas() {
    ...
    drawGroupedConnections(group) {
        ...
        drawArrowByType(this.ctx, { x: targetCenterX, y: arrowStartY }, { x: targetCenterX, y: targetY }, group.type, this.config);
