...
export function calculateOrthogonalPath(from, to, relType, classPositions, usedPaths) {
    const margin = 35;
    const points = [];
    let lowerClass = from;
    let upperClass = to;
    ...
    const obstructingClasses = findObstructingClasses(minX - margin, maxX + margin, Math.min(startY, endY), Math.max(startY, endY), lowerClass, upperClass, classPositions);
    if (obstructingClasses.length > 0) {
        bestMidY = findBestHorizontalY(startX, endX, startY, endY, lowerClass, upperClass, obstructingClasses, classPositions, usedPaths);
    ...
    usedPaths.push({ x1: finalStartX, y1: startY, x2: finalStartX, y2: bestMidY }, { x1: finalStartX, y1: bestMidY, x2: finalEndX, y2: bestMidY }, { x1: finalEndX, y1: bestMidY, x2: finalEndX, y2: endY });
    return points;
}
export function adjustVerticalLineX(x, y1, y2, relatedClass, classPositions) {
...
export function findBestHorizontalY(startX, endX, startY, endY, from, to, obstructing, classPositions, usedPaths) {
    const minX = Math.min(startX, endX);
    const maxX = Math.max(startX, endX);
    ...
    for (const candidateY of candidates) {
        if (candidateY <= minY + 15 || candidateY >= maxY - 15)
            continue;
        const score = calculatePathScore(candidateY, minX, maxX, from, to, margin, classPositions, usedPaths);
        if (score < minScore) {
            minScore = score;
            bestY = candidateY;
            if (score === 0)
...
function calculatePathScore(candidateY, minX, maxX, from, to, margin, classPositions, usedPaths) {
    let score = 0;
    for (const pos of classPositions.values()) {
        if (pos === from || pos === to)
            continue;
...
export function findObstructingClasses(minX, maxX, minY, maxY, from, to, classPositions) {
    const obstructing = [];
    const margin = 15;
...
export function findObstructionBetween(x1, y1, x2, y2, target, classPositions) {
