...
import { removeComments, extractBalancedBraces, getTopLevelStatements } from '../utils/parsingUtils.js';
import { parseVisibility, parseTypeScriptParameters, extractConstructorParams, filterControlFlowKeywords } from '../utils/parserHelpers.js';
export class TypeScriptParser extends BaseParser {
    ...
    parseStreaming(sourceCode, diagram) {
    ...
    findClassMatchesStreaming(code) {
        const matches = [];
    ...
    findInterfaceMatchesStreaming(code) {
        const matches = [];
    ...
    processClassMatch(code, match, diagram) {
        ...
        if (match.extendsClass) {
            ...
            diagram.addRelationship(new Relationship(match.className, cleanParent, RelationType.INHERITANCE));
        }
        if (match.implementsInterfaces) {
            const interfaces = match.implementsInterfaces.split(',').map((i) => i.trim());
            interfaces.forEach((interfaceName) => {
                const cleanInterface = this.cleanTypeSimple(interfaceName);
                diagram.addRelationship(new Relationship(match.className, cleanInterface, RelationType.IMPLEMENTATION));
            });
        }
    }
    processInterfaceMatch(code, match, diagram) {
        ...
        if (match.extendsInterfaces) {
            const interfaces = match.extendsInterfaces.split(',').map((i) => i.trim());
            interfaces.forEach((parentInterface) => {
                const cleanParent = this.cleanTypeSimple(parentInterface);
                diagram.addRelationship(new Relationship(match.interfaceName, cleanParent, RelationType.INHERITANCE));
            });
        }
    }
    parseFields(classBody, classInfo) {
    ...
    parseMethods(classBody, classInfo) {
        const statements = getTopLevelStatements(classBody, { supportRegex: true });
        ...
        statements.forEach(statement => {
            ...
            if (filterControlFlowKeywords(methodNameCore))
                return;
    ...
    addConstructor(statement, classInfo) {
    ...
    parseConstructorAssignments(classBody, classInfo) {
    ...
    parseInterfaceProperties(interfaceBody, classInfo) {
    ...
    parseInterfaceMethods(interfaceBody, classInfo) {
    ...
    parseRelationships(code, diagram) {
            ...
            for (const field of classInfo.fields) {
                ...
                extractedTypes.forEach(extracted => {
                    if (diagram.hasClass(extracted.typeName) && extracted.typeName !== classInfo.name) {
                        ...
                        diagram.addRelationship(new Relationship(classInfo.name, extracted.typeName, relationType));
                    }
                });
            }
            for (const method of classInfo.methods) {
                for (const param of method.parameters) {
                    ...
                    paramTypes.forEach(extracted => {
                        if (diagram.hasClass(extracted.typeName) && extracted.typeName !== classInfo.name) {
                            diagram.addRelationship(new Relationship(classInfo.name, extracted.typeName, RelationType.DEPENDENCY));
                        }
                    });
                }
    ...
    extractTypesFromGeneric(typeStr) {
        const results = [];
