import { Field, Method, Parameter, Visibility, ClassInfo } from '../models/ClassDiagram.js';
import { cleanString } from './stringUtils.js';

export function parseVisibility(visibilityStr: string, defaultVisibility: Visibility = Visibility.PUBLIC): Visibility {
  ...
  if (lower === 'internal' || lower === 'package') return Visibility.PACKAGE;
  return defaultVisibility;
}

export function parseTypeScriptParameters(paramsStr: string): Parameter[] {
...
export function parseJavaScriptParameters(paramsStr: string): Parameter[] {
...
export function parseCppParameters(paramsStr: string): Parameter[] {
  if (!paramsStr.trim()) {
    return [];
  }
...
export function parseCSharpParameters(paramsStr: string): Parameter[] {
  if (!paramsStr.trim()) {
    return [];
  }
...
export function addConstructorToClass(
  paramsStr: string,
  visibility: Visibility,
  classInfo: ClassInfo,
  parseParams: (params: string) => Parameter[]
): void {
...
export function splitFieldDeclarations(fieldsStr: string): string[] {
  const fields: string[] = [];
  let currentField = '';
  let depth = 0;
...
export function parseFieldDeclaration(fieldDecl: string): { name: string; defaultValue?: string; extraPointers?: string } {
...
export function shouldSkipMethod(methodName: string, parentName: string): boolean {
