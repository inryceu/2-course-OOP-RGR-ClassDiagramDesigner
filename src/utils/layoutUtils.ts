...
export function calculateConnectionDensity(
  classNames: string[],
...
export function areClassesOnSameLevel(
  positions: Map<string, Rectangle>,
  classNames: string[],
  threshold: number = 10
): boolean {
  if (classNames.length < 2) return false;
...
export function centerElementsHorizontally(
  positions: Map<string, Rectangle>,
  classNames: string[],
  targetWidth: number,
  offsetX: number = 0
...
export function minimizeCrossings<T extends { name: string }>(
  items: T[],
  getCrossings: (items: T[], i: number, j: number) => number,
...
function distributeRemainingClasses(
  remaining: ClassInfo[], 
  hierarchy: ClassInfo[][], 
